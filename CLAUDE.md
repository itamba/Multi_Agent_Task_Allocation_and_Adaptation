...
| Change the Phase-2 GRAPH reward (terminal utility-regret) | `rl/training/graph_reward.py` → `compute_episode_reward` / `plan_value` / `realized_utility` / `RewardConfig` |
...
**Episode-setup layer (Stage 0): BUILT & REVIEWED** — `rl/training/graph_episode_setup.py` + `rl/training/belief.py`. `setup_episode(scenario_json) -> EpisodeContext` wires env (`gymnasium.make("blade/BLADE-v0", …)`) -> blue-side extract (`create_agents_from_scenario` / `generate_all_enemy_tasks`) -> `split_tasks` (a real discovery-chain rejection sampler, rewritten graph-native — NOT imported from the flat path — that masks a `hidden` subset such that every hidden target keeps a KNOWN neighbour within `detection_km`, pins isolated targets to known, and rejection-samples up to `max_attempts`; so `partial ⊊ full`, A_init covers only known targets, and hidden targets become discoverable pop-ups at runtime) -> `solve_and_normalize` twice (partial -> A_init/`belief_tasks`; full -> oracle, a distinct object) -> N mutually-independent `Belief`s (`deepcopy` tasks + `_copy_solution` solution) -> one `GraphPlanExecutor` on the allocated-only baseline with the single `DETECTION_KM` radius. Belief independence and allocated-only universe are proven in the `_selftest`. Graph-native; imports NOTHING from the flat `train_full.py`. (Exposes `observation`, the reset seed the tick-loop consumes for its first sense.)

**Tick-loop layer (Stages 2-6): BUILT & REVIEWED** — `rl/training/graph_tick_loop.py`. `run_episode(policy, ctx, cfg=None) -> EpisodeResult` drives ONE episode as a strict TWO-PHASE tick: Phase 1 runs every ego's `sensed_target_ids -> decide_triggers -> (on wake) _wake_decision` against the SAME `obs` snapshot with NO `env.step` (pure per-ego belief edits); Phase 2 issues ONE `env.step(executor.next_actions(obs))` for the whole tick. Because BLADE never advances until all egos have decided on the identical snapshot, the Phase-1 ego order cannot affect the outcome — the structural no-comms guarantee (proven in `_selftest`: `env.step` count == tick count). `_wake_decision` is the per-wake chain `build_graph_observation -> GraphEncoder -> ActionHead -> build_action_mask -> sample_action -> apply_meta_action -> executor.resync` under `torch.no_grad`, editing ONLY the acting ego's belief. `Policy` (`build_policy()`) bundles encoder+head, built ONCE and passed in (lives across episodes). The reward/PPO seam is `EpisodeResult.trajectory: List[Transition]` (each wake -> one detached `Transition`; `reward=None` until the reward task fills it). The loop does NOT own the agent lifecycle (executor owns `dead`/`done`/`rtb`/`is_done`) and builds `cfg` once with `detection_range_km == executor.arrival_threshold_km` (the single unified radius). ETA is dormant (`never_overdue`). Loop ends on `is_done` / `terminated` / `truncated`. NOTE: under solver 2:1 stacking a redundant agent chasing an already-killed target never proximity-confirms, so episodes currently end via `truncated` rather than `done` — an executor/solver follow-up, not a loop defect.

**Reward layer (Stage 7): BUILT & REVIEWED** — `rl/training/graph_reward.py`. `compute_episode_reward(ctx, result, cfg=RewardConfig()) -> EpisodeReward` fills the `Transition.reward` seam with an EPISODE-TERMINAL, utility-based reward: `R = (U_achieved − c·U_aircraft·n_lost − U_oracle) / (|U_oracle| + eps_regret)`. `U_oracle = plan_value(ctx.oracle_solution, ctx.oracle_tasks)` — the static full-set optimum, **bit-faithful to the solver objective** (`plan_value` reproduces `MatchAou._add_objective` exactly and REUSES the solver's `EPSILON`; proven in `_selftest` T1 under bonmin). `U_achieved = realized_utility(ctx.oracle_tasks, ctx.executor.done)` — a task pays its full `utility` IFF **all** its steps' targets are confirmed-killed, **deduped over ego** (single-step regime ⇒ per-target; the all-steps form is future-proof). `U_aircraft` = max target utility; `n_lost = len(ctx.executor.dead)`. `c = cfg.aircraft_penalty_coeff` **DEFAULT 0.0** (v1 = pure utility ratio; `c=1.0` makes a lost aircraft cost one max-utility target, so suicide-on-a-target is never net-positive). `eps_regret` (1e-5) is a DIVISION GUARD ONLY — **distinct** from the solver `EPSILON` (probability stabilizer). Placement is **terminal-on-last-wake** (every other transition 0.0; empty trajectory ⇒ nothing attached, breakdown still returned) — the seam the future PPO/GAE task consumes. **No-communication:** this reward is a CENTRALIZED/privileged TRAINING signal — it MAY read global state (`executor.done` spans all egos; the oracle is full-info) but MUTATES ONLY `Transition.reward`, never flows into any belief/observation/policy input, is torch-free/BLADE-free at runtime, and reads via `ctx` ONLY (no locked module touched). Purity is proven in `_selftest` (T6 on stubs + T7 on real objects: `beliefs`/`executor.plans`/`observation` byte-unchanged). **KNOWN v1 ASSUMPTION — `probability=1.0`:** `U_oracle` is an EXPECTED value (`plan_value` discounts by the per-step success factor) while `U_achieved` is a REALIZED outcome (full credit for a confirmed kill); they coincide up to a ~1e-6 `EPSILON` sliver at `probability=1.0` (so `R ∈ [-1, ~0]`) but DIVERGE at `probability<1.0` (a lucky realization can exceed the expectation ⇒ `R` leaves `[-1,0]`) — revisit when p<1 is introduced. Re-solving/re-executing the oracle for a *realized* baseline was **rejected**: a second BLADE episode is expensive and stochastic, and a realized-oracle would flatten the very gradient that teaches stacking-recovery (empirically T7: `U_oracle=240` vs `U_achieved=160` ⇒ `R=−0.33`, the signal to abort the redundant 2:1 agent). **Not built (deliberate seams):** per-wake/dense regret, degraded-state oracle re-solve, centralized critic/value head (encoder `pool()` is the hook), PPO buffer / `evaluate_action` / GAE. `_selftest`: `env PYTHONPATH=src python -m match_aou.rl.training.graph_reward` under `nlp_env` (T1/T7 need bonmin and SKIP if absent; T2–T6 always run).
...
- reward DENSIFICATION + CTDE — the v1 terminal utility-regret reward is BUILT & REVIEWED (`rl/training/graph_reward.py`); still open are per-wake/dense credit, the centralized critic/value head (encoder `pool()` hook), and the operand-scale rework for the `probability<1.0` regime (v1's realized-achieved vs expected-oracle comparison is exact only at `probability=1.0`);
