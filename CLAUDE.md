...
| Change the Phase-2 GRAPH observation (nodes / edges / features) | `observation/graph_builder.py` → `build_graph_observation` (`GraphObservation` with `task_features[k,5]` + `agent_features[a,1]` = `[fuel_norm]` (ego real / peers `0.0`); `GraphObservationConfig`; `EdgeType`) |
...
**Detection/attack range — resolved (executor).** The executor uses a single `arrival_threshold_km` as **see range = attack range = arrival** (50 km). The graph builder's `GraphObservationConfig.detection_range_km` stays a separate, independently tunable value; do not silently switch either to BLADE `aircraft.range`.
