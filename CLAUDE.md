...
- **PPO training loop (next major piece):** variable-size buffer + `evaluate_action` (re-encode stored graph actions with grad; the encoder is single-graph, batching is the buffer's concern) + GAE over the terminal-on-last-wake placement + the outer episode loop that wraps `setup_episode → run_episode → compute_episode_reward → update`.
...
- **BLADE playback recording is a NO-OP on the graph path:** `Game.start_recording / record_step / export_recording` exist but are never driven (the retired flat path drove them; the graph path dropped all three). The harness's `--record-first` only arms the export path. Fix = start once post-load, `record_step` each Phase-2 tick (inside `run_episode` — a locked-file edit), export at episode end. Good small first task before PPO.
- **Delete the retired flat path** once the graph training loop is validated end-to-end (flat `train_full.py` loop, `observation_builder`/`observation_types`, `network.py`, flat `ppo_trainer`/`reward`/`oracle`/`rollout_buffer`/`episode_initializer`). Prerequisites discovered in recon: relocate `SelfState` out of `observation_types.py` first (live graph dep via `self_features`), and strip the flat re-exports from the four `rl/*/__init__.py` (they drag the flat path in at import time of any graph submodule).
