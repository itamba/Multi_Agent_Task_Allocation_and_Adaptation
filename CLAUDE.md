...
- `PENDING` — graph_builder: inline `_compute_fuel_norm` (sever the flat observation seam; Step 1 of the flat-path cleanup).
...
- **Delete the retired flat path** once the graph training loop is validated end-to-end (flat `train_full.py` loop, `observation_builder`/`observation_types`, `network.py`, flat `ppo_trainer`/`reward`/`oracle`/`rollout_buffer`/`episode_initializer`). Prerequisites (Phase-1 inventory, measured): (1) DONE — relocated `_compute_fuel_norm` into `graph_builder`, severing the graph's only **source-level** import from the flat observation package (`self_features`). Closure-neutral by itself — `rl/observation/__init__` still pins the flat chain via `observation_builder → self_features → observation_utils`; the runtime freeing happens in (2) below. (2) Strip the flat re-exports from FIVE __init__ vectors — the four `rl/*/__init__.py` plus `utils/blade_utils/__init__.py` (two dead re-exports) — with an import-purity test: after importing every graph entry module, `sys.modules` contains ZERO flat-only modules (the 16-baseline had a counting gap — Step-1's P2 found the `__init__` → `observation_builder:24` → `self_features` chain Phase 1 missed; the absolute criterion is gap-proof).
