...
- **BLUE strike-weapon lethality is `1.0` (data, not code).** In `strike_training_4v5.json` every BLUE aircraft weapon (AIM-120 / AIM-9 / AGM-65) is set to `lethality = 1.0`, so a launched salvo is a guaranteed kill — this backs the graph executor's **done-on-emit** model (emit attack ⇒ target destroyed), eliminating the ~12% miss from BLADE's stochastic kill (`weaponEngagement.py`: `random_float(0, 1) ≤ weapon.lethality`). RED SAM lethality is left **stochastic and unchanged** (`0.9 / 0.85 / 0.7`) on purpose: probabilistic loss of peers en route is exactly what drives **cooperative recovery (CR)**, so forcing RED to `1.0` would distort the threat model. `scenario_generator` clones weapon stats verbatim (it only reassigns ids / red side tags), so the scenario JSON is the **single source** of lethality. A stochastic-strike enrichment (BLUE `< 1.0`) is a deliberate future option, not enabled now.
