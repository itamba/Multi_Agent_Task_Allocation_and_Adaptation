...
- **A GENERALIZED-V2 benchmark manifest and two development-profile R1 arms (actor-only and
  CTDE) were executed at measured code SHA `ae42cb01677f94868b2873008d87be677e31f0c8`** and are
  preserved in the open evidence PRs #61 and #62 (§4). The research decision that authorized
  them, and any conclusion drawn from them, is not recorded in the repository (§7).
...
| GENERALIZED-V2 benchmark manifest | `ae42cb01…` (per both run configs) | `manifest_id ef17a68a1d41b04cf6cb9b4ed92d91f3a687b600376ff1dc7bd5b83b21a46ea8`; file SHA-256 `dd72afc9cc0d2d1fe494ddbebe53734dc36bd5890997125d3e96a2a59641a103` at the external path `C:/Users/Itama/PycharmProjects/graph_rl_v2_benchmark_preflight_seed2000000_ae42cb0/benchmark_manifest.json` (recorded in PR #62); **not committed; not inspected by GPT; no review record in the repository** | [measurements §7](docs/history/measurements.md#7-generalized-v2-development-r1-arms) |
| GENERALIZED-V2 development R1 — actor-only | `ae42cb01677f94868b2873008d87be677e31f0c8` | **no verdict recorded** in PR #61 or its `artifact_sha256.txt` | evidence PR #61 |
...
repository and population measurements with no causal solver-quality inference; approved
baselines are reused as they are and never rerun, repaired, resumed or retuned; the old
fixed-cell CTDE measurement stays out of scope unless the user explicitly asks
([`experiments.md`](docs/workflows/experiments.md)).
...
- any further V2 benchmark preflight, or replacing, regenerating or re-identifying the manifest
  the preserved arms used;
- any further V2 training or development evaluation, any confirmatory-profile evaluation, and
  re-running either preserved arm;
- any R1 rerun, repair, resume or extension; any P1 rerun; resuming, repairing or extending the
  aborted P1 arm;
- a new control arm, further CTDE arms, the five full cluster runs, and retuning;
...
- No verdict is recorded for the V2 actor-only development arm; the CTDE verdict is attributed
  to PR #62's own text only.
- The authorization and review record for the V2 benchmark preflight is absent; the external
  manifest was not inspected, and whether confirmatory data was used is not inferred.
