...
- **OBSERVATIONAL EPOCH-0 ACTOR-GRADIENT REPORT (CTDE only, opt-in).** `CTDEUpdater.update` also
  ...
  `ActorGradientReport` of flat numpy gradients. No forward, GAE pass, RNG draw, `.grad` write or
...
| change the epoch-0 actor-gradient report | `rl/training/graph_ppo.py`: `ActorGradientReport`, `GradientSink`, `_flat_actor_grad`, the `gradient_group_ids` / `gradient_sink` parameters of `CTDEUpdater.update`; tests `tests/test_graph_ctde_actor_gradient_diagnostics.py` | §4; [artifacts and metrics §5.2](artifacts_metrics.md#52-ctde-actor-gradient-diagnostics) |
