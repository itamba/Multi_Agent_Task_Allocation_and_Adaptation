...
| 2026-09-19 | **EXPLICIT ACTING-EGO READOUT IMPLEMENTATION OPENED** on the same branch / PR #75, append-only after the role-only verdict documentation, transport `GPT_GITHUB` | Changes the locked CTDE critic contract ([policy and CTDE §4](../contracts/policy_ctde.md#4-phase-b-ctde)) in its readout only: `GraphEncoder.pool_with_ego` returns the global mean pool and the projected post-message-passing acting-ego row from one encoder pass, and `ValueHead` consumes `[mean pool ; ego embedding]` (`2 * embed_dim`); the critic refuses a state without a valid acting-ego agent node; CTDE checkpoints carry `critic_readout_id`, and an older `embed_dim`-wide critic head cannot load. Central features, role conditioning, actor, action representation, reward, GAE, PPO, entropy, learning rates, optimizers, clipping, population and FD mechanics unchanged. The role-only measurement (§12 of `measurements.md`) stays a description of `68055e3…`. **No scientific run is authorized** |
| 2026-09-19 | **EXPLICIT ACTING-EGO READOUT DEVELOPMENT DIAGNOSTIC AUTHORIZED, EXECUTED AND REVIEWED** — `graph_rl_v2_explicit_ego_readout_ctde_fd100_r1_seed3000000_1a1e0c9` at measured SHA `1a1e0c953c54e9d3f46c871158d5ab6bdd881f24` (the GPT-approved explicit-readout candidate on PR #75), under its recorded `authorized_plan.json` (SHA-256 `691cccc5189686f312830afe9154dbe32279a4bf4e213b1f98777cfc00f37d3c`); primary comparator the role-only run. GPT verdict **`APPROVE — VALID DEVELOPMENT DIAGNOSTIC MEASUREMENT`** on the evidence package reviewed at `557e072b94884bef37ece82a064ad877a5a2f636` ([`measurements.md` §13](measurements.md#13-generalized-v2-explicit-acting-ego-readout-ctde-development-diagnostic-and-owner-transition-audit)) | **Historical authorization scope:** that one run only; it is **spent**. **The mean-pool dilution hypothesis is NOT supported:** the explicit `[mean pool ; acting-ego embedding]` readout improved neither behaviour (held-out separation effectively zero throughout, 0 / 20 switches) nor locality relative to role-only (nonterminal median `|td|/|A|` `0.0125` versus `0.0470`; future-dominant fraction `0.959` versus `0.921`). Development only; no confirmatory claim |
| 2026-09-19 | **OWNER-TRANSITION AUDIT RECORDED; ROLE-ONLY `gae_lambda = 1.0` DIAGNOSTIC DECIDED** (user-approved packet, on the review above) | The read-only audit preserved in the explicit-readout package shows, under role-only conditioning, a larger one-step value discontinuity when the next global decision belongs to another ego (median `|td|/|A|` `0.0862` versus `0.0275` same-ego), with a positive within-update `SEVERE − MILD` `ΔV` and a negative future component on those transitions: cross-owner bootstrapping is a real structural feature of the global decision sequence — **descriptive, not evidence that owner switching causes collapse**. Decision: test it with one role-only run at `68055e3…` changing only `ctde.gae_lambda 0.95 → 1.0`, from an isolated detached worktree, under its own authorized bounded plan |
| 2026-09-19 | **ROLE-ONLY `gae_lambda = 1.0` DEVELOPMENT DIAGNOSTIC EXECUTED AND REVIEWED** — `graph_rl_v2_role_only_ctde_lambda100_fd100_r1_seed3000000_68055e3` at measured SHA `68055e39768d5fa601e5960a9f08823b9e65c08f`, under its recorded `authorized_plan.json` (SHA-256 `f47ffcd31874b685effd1ccbc5c5cd2d29eedcbd9a6847dff706ca84cb091731`); same-code comparator role-only λ0.95. GPT verdict **`APPROVE — VALID DEVELOPMENT DIAGNOSTIC MEASUREMENT`** on the evidence package reviewed at `1076208b68ee9abd76f159535c5f38fe98970ce5` ([`measurements.md` §14](measurements.md#14-generalized-v2-role-only-acting-ego-ctde-gae_lambda--10-development-diagnostic)) | **Historical authorization scope:** that one run only; it is **spent**. With `γ = 1` and terminal-only reward the λ = 1 telescoping is exact over all 2981 transitions (max error `5.6e-17`), so no intermediate — including cross-owner — value survives in the actor's total advantage; held-out separation nevertheless stayed effectively zero (update 75 `+0.0000168`, update 100 `+0.0000014`, 0 / 20 switches) while critic severity sensitivity rose. **Intermediate cross-owner bootstrap is not a sufficient explanation**; λ = 1 is a **negative development intervention** and not the final configuration |
| 2026-09-19 | **FINAL CODE STATE OF PR #75: ROLE-ONLY CONDITIONING, MEAN-POOL READOUT, `gae_lambda = 0.95`** (user-approved closure packet) | Supersedes the 2026-09-19 row "EXPLICIT ACTING-EGO READOUT IMPLEMENTATION OPENED" as to the final code: the explicit readout (`GraphEncoder.pool_with_ego`, `CRITIC_READOUT_ID`, the `2 * embed_dim` value head, `critic_readout_id`) is **retired** by an append-only restoration of the reviewed role-only implementation at `68055e39768d5fa601e5960a9f08823b9e65c08f`; it survives only in history and evidence. Role-only acting-ego conditioning is retained as the semantically correct CTDE decision-state definition; the default `CTDEConfig.gae_lambda` stays `0.95`. The acting-ego / critic-locality diagnostics are complete ([`measurements.md` §15](measurements.md#15-acting-ego--critic-locality-development-investigation--closing-interpretation)); no run is active or authorized; the confirmatory profile is untouched; merge awaits GPT exact-candidate review and explicit user authorization |
| 2026-09-19 | **FUTURE RESEARCH DIRECTION: ACTOR-SIDE OPTIMIZATION / STABILITY** (user-approved closure packet) | Current development evidence does not support critic conditioning, mean-pool dilution or cross-owner GAE bootstrapping as a sufficient or primary explanation of the remaining acquisition / retention failure. The next research area — for a future orchestrator — is actor-side optimization / stability, gradient-to-policy mapping and retention mechanics, **not** another critic, readout or λ intervention. No actor-side mechanism is identified; this row authorizes no run or code change |
| 2026-09-19 | **PR #75 INTEGRATED** — the final candidate, reviewed by GPT at exact head `b3a2350f437afee1c66d4fd5a5efa7cb7a353787`, was merged into `main` as `ed33b7e24a652fa00b13c708517012f8b3302496` under user authorization; branch `task/ctde-acting-ego-conditioning` is absent afterwards | Supersedes the pending-review / pending-merge state of PR #75 recorded in the 2026-09-19 rows above, which were true when written. **Current code:** CTDE role-only acting-ego critic conditioning, the unchanged mean-pool critic readout, and default `CTDEConfig.gae_lambda = 0.95`. The explicit `[mean pool ; acting-ego embedding]` readout remains **historical / evidence only** ([`measurements.md` §13](measurements.md#13-generalized-v2-explicit-acting-ego-readout-ctde-development-diagnostic-and-owner-transition-audit)); the three acting-ego evidence packages are on `main` |
| 2026-09-19 | **SEMANTIC-ACTION CTDE DEVELOPMENT R1 RETROSPECTIVELY REVIEWED — `APPROVE — VALID DEVELOPMENT MEASUREMENT`** (GPT verdict after exact-evidence review, as transferred in the user-approved documentation packet) — run `graph_rl_v2_semantic_action_ctde_dev_r1_seed3000000_8056266`, measured code SHA `8056266cff89f677911462b29970346bed0a57c1`, evidence PR #73 @ `ad9b545034670a7c7a9d8ff98012d56c0be07f46` ([`measurements.md` §16](measurements.md#16-generalized-v2-semantic-action-ctde-development-r1--retrospective-review-closure)) | **Development only**; the confirmatory profile untouched. Key finding: the symmetric semantic CTDE arm **did not reproduce the strong transient severity separation of semantic actor-only in the measured evaluation trajectory** (0 / 20 directional switches at every one of the 16 evaluation rounds; maximum evaluated macro ≈ `4.57e-5`; final macro ≈ `+1.46e-7`) — the evidence does not show acquisition of that transient — so **no CTDE benefit is established**; its finer GAE credit did not yield observed severity-conditioned selected-action behaviour. No causal claim. The verdict is retrospective: it does not change the historical statement of [`measurements.md` §11.10](measurements.md#1110-limitations-and-non-claims) that the run was unreviewed and unused there. PR #73 stays an open evidence PR, not for merge, until its lifecycle is separately authorized |
| 2026-09-19 | **RECENT RESEARCH RECORD ALIGNED BEFORE A SEPARATE ARCHIVAL / CLEANUP TASK** (user-approved documentation packet) on branch `docs/recent-research-cleanup-closure` from `ed33b7e24a652fa00b13c708517012f8b3302496`, transport `GPT_GITHUB` | Documentation only: this row, [`measurements.md` §16](measurements.md#16-generalized-v2-semantic-action-ctde-development-r1--retrospective-review-closure), the handoff and the cleanup registry ([`environments_cleanup.md` §4](../workflows/environments_cleanup.md#4-authorized-cleanup)) are brought into agreement with the observed state. **This task authorizes no archive move, PR closing, branch deletion, worktree or scratch removal, or scientific execution**; the archival / cleanup of evidence PRs #71 and #73, their source runs, the diagnostic run directories and the merged PR #74 branch needs its own explicit authorization |
| 2026-09-19 | **RECENT-RESEARCH ARCHIVAL AND GIT CLEANUP PERFORMED** under explicit user authorization, after PR #76 was merged as `67cd12a438452aadf61f312e68484f26dfa1e7c7`: the two semantic-action R1 run directories and the five CTDE development diagnostic run directories were archived into `C:\gra\` by identity-verified same-volume rename (index and move ledger 28 → 35 rows; `ARTIFACT_INDEX.jsonl` SHA-256 `9f31e34652b2b72ad3992289393312dcceee9c68dbbb2931b82553230c76a756`, `ARCHIVE_MOVE_LEDGER.jsonl` `322d821f088ccb0a6b90241253412cadc0b76307224bd1becb251acc2f3bfae3`, `ARTIFACT_INDEX.md` `ec12587c27c4c4c4ad58f12189c932f09638f51821b74e29bc9e429606546e14`; [`measurements.md` §9.5](measurements.md#95-archival-extension-of-2026-09-19)); evidence PRs #71 @ `0d136fa89286c4bbd9e89dfb6bd0a3326c70b670` and #73 @ `ad9b545034670a7c7a9d8ff98012d56c0be07f46` were closed without merge and their branches deleted; the merged branches of PRs #74 and #76 were deleted; authorized main-checkout scratch was removed. Recorded on branch `docs/recent-archive-cleanup-closure`, transport `GPT_GITHUB` | Supersedes the pending-archival / open-evidence-PR state recorded in the rows above, which was true when written. Implements the 2026-09-15 lifecycle decision for these refs and runs. **Nothing is lost:** the §10 and §16 conclusions are durable on `main`, and every archived file is SHA-256-identical to its pre-move bytes, with embedded historical paths not rewritten. The two semantic R1 credit streams were found renamed since their evidence commits (bytes identical; not reverted; recorded in the index). The protected refs are unchanged. Still undecided and untouched: `src/match_aou/rl.zip`, `legacy/run_capture.log`, `src/match_aou/rl/observation/rollouts/`, the `flat-baseline` outputs and the empty `C:\gruns\` ([`environments_cleanup.md` §4.7](../workflows/environments_cleanup.md#47-cleanup-already-performed)). No scientific execution is authorized |
| 2026-09-24 | **ACTOR MISSION-FUEL-SLACK INTERVENTION AND ONE DEVELOPMENT RUN AUTHORIZED** (user-approved FIRST intervention of the actor-side research area; GPT packet `CC_Actor_Mission_Slack_Dev_R1`, Grade A, transport `GPT_GITHUB`, branch `task/actor-mission-fuel-slack-dev-r1` from `3d05a6ca76a0cbaf5c182d35da8677e7c06f6268`) — exactly ONE new actor input, `mission_fuel_slack_norm = (current_fuel − estimated_remaining_mission_and_return_fuel) / max_fuel` on the ego agent row (actor observation `actor_graph_task6_agent2_fuel_norm_mission_fuel_slack_v1`, [policy and CTDE §1](../contracts/policy_ctde.md#1-graph-observation-stage-3)), and exactly ONE fresh actor-only 375-update `generalized_v2` development run against the reviewed semantic actor-only R1 as a cross-version comparator | **TASK-SPECIFIC WORKFLOW EXCEPTION:** the user requested one end-to-end task (implementation, engineering validation, training, evidence) with GPT review only at the end, which waives the intermediate implementation-review stop of [`cc_review.md` §3](../workflows/cc_review.md#3-transport-gpt_github) **for this task only**; the standing workflow is unchanged. Implementation and measurement stay **UNREVIEWED** until GPT reviews the final exact candidate and the measured commit, and passing tests are not approval. The actor encoder's agent input width becomes 2 (no other model change); task features, topology, reachability, mask, action semantics, PPO, critic, reward, population and FD mechanics are unchanged; historical checkpoints do not load into the new width and no conversion exists. No merge, confirmatory evaluation, further arm, replication, tuning, rerun, resume, extension or cleanup is authorized; a failed run is preserved as incomplete, never repaired and restarted |
| 2026-09-25 | **PR #79 REVIEWED AND INTEGRATED** (GPT review outcome, as transferred in the packet `CC_Actor_Credit_Update_Diagnostic_R1`) — the mission-fuel-slack implementation is **APPROVED at exact head `26f8bff97b1cbbca64d18626625d2321545080af`**, and its one run `graph_rl_v2_actor_mission_slack_dev_r1_seed3000000_3bc9441` (measured SHA `3bc944119da08af8e25268c9ee83fc63a8d1e533`) is **reviewed as valid negative development evidence**. The earlier exact-head review of `ea60ce38e9f1b91fb3cb736bb4dba01f88f92523` (CHANGES_REQUESTED, 2026-09-24) stands as history. PR #79 was merged normally as `bcb1746fbc677b3109f73b36699ac3b3780c32a4` (parents `3d05a6ca76a0cbaf5c182d35da8677e7c06f6268`, `26f8bff…`), whose tree equals the approved head tree `d3a0306e57b8390952a68c521c02a418252ae613` | Supersedes the "UNREVIEWED" status of the 2026-09-24 row above. Measured, evidence and review heads stay distinct: measured `3bc9441…`, evidence commit `50c13ee9839a822b12e42fbfed76c8ed55e1f901`, first reviewed head `ea60ce3…`, approved head `26f8bff…`, merge `bcb1746…` ([measurements §17](measurements.md#17-generalized-v2-actor-mission-fuel-slack-development-r1--reviewed)). Branch retirement verified on 2026-09-25: no open PR, the remote branch `task/actor-mission-fuel-slack-dev-r1` absent, no local branch of that name; the measured source worktree `C:/gms1src` (detached at `3bc9441…`) is retained, and no archival of the original run directory is authorized |
| 2026-09-25 | **ACTOR-ONLY CREDIT-TO-UPDATE DIAGNOSTIC R1 AUTHORIZED** (user-approved bounded actor-side investigation; GPT packet `CC_Actor_Credit_Update_Diagnostic_R1`, Grade A, transport `GPT_GITHUB`, branch `task/actor-credit-update-diagnostic-r1` from `bcb1746fbc677b3109f73b36699ac3b3780c32a4`) — opt-in observational every-epoch instrumentation of the actor-only `PPOUpdater` (`train_actor_step_diagnostics.jsonl`, [artifacts and metrics §5.3](../contracts/artifacts_metrics.md#53-actor-only-step-diagnostics)) and exactly ONE fresh 100-update actor-only `generalized_v2` development diagnostic against the first 100 updates of the reviewed mission-slack run | **TASK-SPECIFIC WORKFLOW EXCEPTION:** as for the 2026-09-24 task, the user's preference for end-to-end execution waives the intermediate review stop of [`cc_review.md` §3](../workflows/cc_review.md#3-transport-gpt_github) **for this task only**; the standing workflow is unchanged. Observation, topology, masks, action semantics, reward, returns, advantage normalization, loss reduction, optimizer, hyperparameters, population and FD mechanics are unchanged; no critic is constructed. Implementation and run stay **UNREVIEWED** until GPT reviews the exact candidate; PR #79's approval is not inherited. No second run, restart, resume, replication, extension, tuning, reward / credit redesign, observation change, confirmatory evaluation, cleanup or merge is authorized; an unfavourable result triggers no further arm |
| 2026-09-25 | **PR #80 EXACT-HEAD REVIEW: CHANGES_REQUESTED** (GPT review of `5f5e22ae5f3c108af0a24e18936a87fb5c31dd7b`, packet `CC_PR80_Review_Fixes_R1`) — no demonstrated change to the PPO learning computation; original-trajectory consistency UNRESOLVED | Recorded as protocol deviations, not retroactive authorizations: **D1** — the executed precommitted stop rule (stop only on a `policy_side` FIRST divergence of a combined evaluation-then-training stream) was narrower than the dispatch's "a material unexplained discrepancy stops the run"; **D2** — the final comparison loaded the new and original checkpoints with `torch.load` (offline tensor comparison) although the dispatch said save only, no historical checkpoint loading. The historical comparator and launcher are evidence of what ran, not a reusable guard. Bounded fixes F1–F3 are additive commits on PR #80 (documentation, a hash-pinned source extract, review-derived arithmetic); no `src` / `tests` / config change, no training, evaluation, replay, inference or checkpoint loading. No merge or cleanup authorized; a new scientific intervention needs its own task |
| 2026-09-25 | **PR #80 APPROVED AND INTEGRATED** (GPT exact-head approval, packet `CC_PR80_Approval_and_Merge_R1`, as recorded in the PR description and transferred in the user-approved packet `CC_Flow_Credit_Handoff_Update` Revision 7; recorded 2026-09-26) — **APPROVED at exact head `eb6401167390513f4dc56a946ea76cc3b314607d`** relative to base `bcb1746fbc677b3109f73b36699ac3b3780c32a4`; merge and task-branch deletion explicitly authorized by the user; merged normally as `2d5719370d17594297111350007dd0c8bedb54e1` (second parent `eb64011…`); remote branch absent (verified 2026-09-26) | Supersedes the pending re-review and no-merge state of the 2026-09-25 rows above, which were true when written. **The approval is limited** to the instrumentation implementation and the preservation of **qualified descriptive development evidence** ([`measurements.md` §18](measurements.md#18-generalized-v2-actor-only-credit-to-update-diagnostic-r1--executed-unreviewed), whose own status line predates this row and is not rewritten). Original-trajectory consistency stays **UNRESOLVED**; deviations **D1** and **D2** stay recorded and are not retroactively authorized; no causal mechanism is attributed. The run's authorization is spent. No local cleanup is recorded: the measured worktree `C:/gcud1src` and the original run directory are retained |
| 2026-09-26 | **FLOW / CREDIT RESEARCH QUESTIONS RECORDED** (user-approved documentation packet `CC_Flow_Credit_Handoff_Update` Revision 7, final; transport `GPT_GITHUB`, branch `docs/flow-credit-research-handoff` from `2d5719370d17594297111350007dd0c8bedb54e1`) — the user's flow / credit / batch / PPO / evaluation walkthrough is closed; six parent research questions (REWARD-01, CREDIT-01 with CREDIT-02 and conditional RECORD-01, BASELINE-01, CRITIC-01, HEAD-01, OBS-01 with OBS-02) and the closed dispositions REWARD-00, PEER-01 and SCALE-01 are recorded in the handoff §5.1 | **Documentation only.** Questions, not diagnosed causes or approved fixes: no reward coefficient, credit rule, estimator, critic change, architecture or experiment is selected, and `gamma = 1` stays the user's requirement. The questions are unassigned and unstarted; every future research chat starts read-only, and any implementation or run needs its own authorized bounded plan. CRITIC-01 is a read-only review, not the critic / readout / `lambda` intervention the 2026-09-19 direction excluded. This row authorizes no code, configuration, training, evaluation, replay, checkpoint loading, cleanup or merge |
